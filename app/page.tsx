...
export default async function Home({
  ...
  const [
    ...
    getRedditPopularProducts(4), // Show 4 products
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-20">
        ...
        {searchQuery ? (
          ...
          <>
            ...
            {/* Section 2.5: Beauty Products Popular on Reddit */}
            {redditPopular.length > 0 && (
              <section className="mb-20">
                <div className="mb-8">
                  <div>
                    <h2 className="text-3xl md:text-4xl font-bold text-[#2D2D2D] mb-3 tracking-tight">
                      Beauty Products Popular on Reddit
                    </h2>
                    <p className="text-sm text-[#6b6b6b] tracking-wide">
                      Products that are trending in Reddit beauty communities with high engagement and multiple mentions.
                    </p>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
                  {redditPopular.map((product) => (
                    <ProductCard key={product.id} product={product} />
                  ))}
                </div>
                <div className="text-center">
            ...
            {/* Section 2.6: Most Recent Trending Products */}
            {mostRecent.length > 0 && (
              <section className="mb-20">
                <div className="mb-8">
                  <div>
        ...
        {/* Empty State */}
        {!searchQuery && trendingNow.length === 0 && risingFast.length === 0 && recentlyHot.length === 0 && redditPopular.length === 0 && mostRecent.length === 0 && (

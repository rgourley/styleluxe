...

{
  ...
  "alternatives": "Alternatives section with 3 options. CRITICAL: Each alternative MUST be on its own line with proper formatting. Use this EXACT format (each on a separate line with double line breaks between them):\n\n**Budget:** Product Name ($price): explanation of tradeoff and why it's different\n\n**Luxury:** Product Name ($price): explanation of tradeoff and why it's worth the extra cost\n\n**Different approach:** Product Name ($price): explanation of why this is a different approach and when it makes sense\n\nIMPORTANT: Format as plain text with product name and price in parentheses followed by a colon, then the explanation WITHOUT a leading dash. The system will automatically add Amazon links. Each alternative must be on its own line with a blank line between them. DO NOT start the explanation with a dash (-).",
  "verdict": "The Verdict - A concise summary that synthesizes everything discussed above. This is a FINAL VERDICT/SUMMARY, not an intro. DO NOT start with questions like 'Is it worth the hype?' Instead, write a conclusive summary that wraps up your analysis. Example: 'Look, this isn't going to replace retinol for actual anti-aging. But for temporary glow and the fun factor? It delivers.' OR 'After analyzing the reviews and data, this is a solid option for [specific use case] at ${priceText}. It won't work for everyone, but if you're [target user], it's worth trying.' Write 2-3 sentences that summarize the key points and give a clear conclusion.",
